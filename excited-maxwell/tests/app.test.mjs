...
test('keeps the timer, shop, and spontaneous idea place in the app', () => {
  assert.match(markup, />Timer<\/h2>/);
  assert.match(markup, />Shop<\/h2>/);
  assert.match(markup, />Spontaneous Idea Place<\/h2>/);
  assert.match(source, /function startTimer\(\)/);
  assert.match(source, /const SHOP_ITEMS = \[/);
  assert.match(source, /function sparkIdea\(\)/);
});

test('bee plank, roulette wheel, and restored panels have presentation styles', () => {
  assert.match(styles, /\.task-plank/);
  assert.match(styles, /\.roulette-wheel/);
  assert.match(styles, /@keyframes spinWheel/);
  assert.match(styles, /\.timer-panel/);
  assert.match(styles, /\.shop-panel/);
  assert.match(styles, /\.ideas-panel/);
});
